...
async function descMore() {
  const authConn = await mysql.createConnection({ host: 'localhost', port: 3306, user: 'root', password: 'root', database: 'vizito_auth' });
  const [cols] = await authConn.execute('DESCRIBE doctor_profiles');
  console.log('doctor_profiles cols:', cols.map(c => c.Field));
  await authConn.end();
}

descMore().catch(console.error);
